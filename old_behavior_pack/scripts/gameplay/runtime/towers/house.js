...
class Home {
    ...
    upgrade() {
        tier++
        Home.upgradeCallbacks.forEach(x => x(tier))
    }
}
...
system.runInterval(() => {
    ...
    for (const player of world.overworld.getEntities({ location: { x: 75, y: 62, z: 115 }, maxDistance: radius })) {
        player.health = Math.min(player.getMaxHealth(), player.health + 2)
    }
...
{
    let lost, query = { type: "dest:objective" }, query2 = { maxDistance: 25, families: ["enemy"] }
    system.runInterval(() => {
        const location = world.overworld.getEntities(query)[0]?.location
        query2.location = location
        for (const mob of world.overworld.getEntities(query2)) {
            world.hp -= 1
            mob.applyDamage(2)
            if (0 >= world.hp && !lost) {
                lost = true
